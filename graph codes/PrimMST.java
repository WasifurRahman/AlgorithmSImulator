...
public class PrimMST{
    ...
    public void rollTheBall(){
        ...
        while(!pq.isEmpty()){
            int v=pq.delMin();
            visit(graph,v);//add closest vertext to the tree
        }
    ...
    private void visit(EdgeWeightedGraph G,int v){
        ...
        marked[v]=true; 
        textMessage="Vertex of minimum distance from the MST is :"+v +" which has a distance of :"+distTo[v];
        //vertexBlink for a while
        vertexBlink(v,Color.RED);
        pause();
        textMessage="So vertex "+v+" is in the MST";
        vertexMessage="Current Vertex :"+v;
        pause();
        //now v has been added to the mst..so paint v RED
        // changeVertexColor(v,Color.RED,1.5);
        //the edgeTo[v] is an mst edge.so paint it black unless the vertex is 0
        //hence the edge is null;
        if(v!=0){
            //edgeBlink(edgeTo[v],Color.RED);
            textMessage=edgeTo[v] +"  is now an MST edge";
            mstMessage="MST Edges:  ";
            //add the mst Edges to the String
            mst.add(edgeTo[v]);
            for(Edge e:mst)mstMessage+=e+"  ";
            changeEdgeColor(edgeTo[v],Color.BLACK,0.5);
            pause();
        ...
        for(Edge e:G.adj(v)){
            ...
            if(!mst.contains(e)){
                ...
                changeEdgeColor(e,Color.YELLOW,0.5);
                pause();
            }
            int w=e.other(v);
            if(marked[w]){
                ...
                if(!mst.contains(e)){
                    textMessage="vertices "+v+" "+"and "+w+" are in the MST.So the edge "+e+" cannot be in the MST";
                    changeEdgeColor(e,Color.GRAY,0.5);
                    pause();
                }
            ...
            if(e.weight()<distTo[w]){
                ...
                if(pq.contains(w)){
                    ...
                    //first blink the new edge
                    textMessage="A better edge  " + e + " connecting vertex "+w+" to the MST has been found";
                    edgeBlink(e,Color.YELLOW);
                    //panel.repaint();
                    pause();
                    ...
                    textMessage="The new best edge for vertex "+w+" is "+e;
                    changeEdgeColor(edgeTo[w],Color.RED,0.02);
                    pause();
                    //then set the new priority near the vertice
                    textMessage="So the priority of the vertex "+w+" is decreased from "+prevDist+" to "+distTo[w];
                    ...
                    changeVertexColor(w,Color.ORANGE,0.25);
                    pause();
                    //then the change of the best matches edge to it to RED
                    textMessage=edgeTo[w]+"  is the best edge connecting vertex "+w+" to the MST";
                    changeEdgeColor(edgeTo[w],Color.RED,0.25);
                    pause();
...
//            
            ...
            for(int i=0;i<SIZE;i++){
                ...
                if((co_ords[i].getColor()==Color.ORANGE)||(co_ords[i].getColor()==Color.RED)){//if the vertex is on the pq or mst
                    g.drawString(i+" | "+distTo[i],co_ords[i].get_X()-60,co_ords[i].get_Y()+5);
                }
                else{
